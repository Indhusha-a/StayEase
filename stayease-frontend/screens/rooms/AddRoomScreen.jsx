...
// Field is defined outside the screen component so React does not
// destroy and re-create it on every state change — this keeps the keyboard open while typing
...
export default function AddRoomScreen({ navigation }) {
  const [form, setForm] = useState({
    ...
    capacity: '', description: '', floor: '', availabilityStatus: 'available',
  });
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') return Alert.alert('Permission needed', 'Allow gallery access to upload images.');
  ...
  // Sends the form as multipart/form-data so the image file is included
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (image) {
        data.append('image', { uri: image.uri, name: 'room.jpg', type: 'image/jpeg' });
      }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <View style={styles.headerCard}>
        ...
        <Text style={styles.headerSub}>Fill in the room details below</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  imagePickerText:  { color: '#9CA3AF', marginTop: 8, fontSize: 13 },
